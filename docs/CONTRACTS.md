...
Commit `345fa7b87d0b5f332038140ce8d0e06d3faa9df3` replaces `ORB-C12` and
refreshes `ORB-C13` with bounded PTY process-group cleanup. Orbit starts in an
ordinary Linux environment without delegated cgroup access, performs no delayed
forced foreground-group signal after reaping a shell that exits during grace,
and still requires direct-child reaping before successful Stop. Focused
lifecycle, canonical Rust, governance, consumer-import, Clippy, and diff checks
passed. ORBF, ORBS, and direct dependencies are unchanged.
