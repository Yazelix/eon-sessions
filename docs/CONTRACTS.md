...
Commit `0ca0cc93b83793d08eadcca7eac6e947a80ce25d` hardens `ORB-C12` by
waiting at most one second for external launcher cgroup placement while
preserving the exact parent trust and fail-before-PTY-exec checks. The focused
regression, canonical Rust, governance, consumer-import, Clippy, and diff checks
passed, and an exact COSMIC launch placed Orbit and its PTY child in the
expected app and per-Session scopes. Protocols and dependencies are unchanged.
