...
Commit `6bcd9dbcc55acaf933c8005ad9e399baaeabe770` hardens `ORB-C2`, `ORB-C4`,
`ORB-C5`, `ORB-C6`, `ORB-C8`, and `ORB-C9` with the focused terminal
conformance corpus. The focused and canonical Rust, governance, consumer-import,
Clippy, and diff checks passed at that exact revision. Product behavior, wire
compatibility, dependencies, and owned Rust LOC are unchanged. `ORB-C1` remains
proved at `9c0617a97612cdd045ed67b5cd7c87244eb888e6`.

The working `ORB-C12` candidate changes the shared Linux PTY launch and shutdown
surface. Its focused real-PTY success and pre-spawn failure checks pass, but it
has no accepted immutable proof revision. The Session cgroup is a lifecycle
scope, not a hostile-code sandbox: a same-user process can move itself into an
external cgroup, at which point it is outside Orbit's membership result. This
known limit defines the contract rather than blocking it. Cgroup handoff does
not detach a process from its controlling terminal, so ordinary kernel hangup
semantics still apply until the process leaves that terminal session. The
existing `ORB-C1` through `ORB-C11` entries therefore describe their last
accepted proofs only; the candidate classifies them as preserved but does not
refresh or widen those proofs.
...
| `conformance_c4_real_pty_reattach_converges_through_complete_ordered_frames` | Primary pending wrap, rich alternate screen, resize, detach under output, then split CSI, UTF-8, and APC tails | Reattachment starts at the complete current frame; later revisions strictly increase and converge on the final primary state | Exact reattached-frame equality and revision order; semantic final convergence | `ORB-C4` |
