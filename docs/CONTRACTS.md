...
| `ORB-C2` | Orbit is the sole libghostty terminal-state owner and the only component that sends terminal-generated responses to the PTY | Orbit platform-neutral authoritative owner loop | Proved | `f4f0b0a82333d088ad40e2b63108d4905466e8f2` | [`ansi_palette_is_bounded_and_resets_to_supplied_defaults`](../src/main.rs) checks bounded startup parsing and libghostty-owned default, override, and reset semantics; [`parser_state_and_terminal_replies_survive_client_failure`](../tests/lifecycle.rs) checks retained parser state and ordered terminal replies; [`only_authoritative_terminal_emits_pty_responses`](../tests/formatter_attachment.rs); canonical Rust verification suite | Ordered client-visible effects remain a separate unproved transport gap |
...
| `ORB-C6` | The presentation boundary carries rich terminal state or explicitly declares unsupported capabilities; it never silently collapses the contract to plain text | Orbit authoritative extraction plus canonical `orbit-protocol` values and codecs | Proved | `f4f0b0a82333d088ad40e2b63108d4905466e8f2` | [`ansi_palette_launch_is_visible_and_survives_reattachment`](../tests/lifecycle.rs) checks the supplied palette in the first complete frame, equal reattachment, OSC override and reset, unchanged index 16, and rejection before child start; the real-terminal corpus in [`src/presentation.rs`](../src/presentation.rs) checks exact rich presentation; strict canonical ORBF-in-ORBS codec coverage; canonical Rust verification suite | Version 1 explicitly leaves Kitty graphics unsupported; expanding that capability requires a later user decision |
