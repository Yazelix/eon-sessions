...
`ORB-C12` is partially proved while
`orb-portable-pty-shutdown-ke1` replaces the accepted Linux containment
requirement with bounded PTY process-group cleanup. The user accepted the
weaker whole-tree guarantee on 2026-08-24 so Orbit can start in an ordinary
Linux environment without delegated cgroup access. ORBF, ORBS, and direct
